...
class HttpHandler(RapidBaseHttpHandler):
    '''The original http handler, used by the httptester app.
       ...
       Note about unicode: unicode data should be encoded as %uXXXX
       where 'X' represents an arbitrary hex value. This is consistent
       with Javascript's 'escape' encoding. Reserved characters, as always,
       are encoded '%XX' (e.g. %20). If you need '%' in your data, you need
       to percent-encode it as %25
    '''
    
    msg_store = {}
    
    def do_GET(self):
        """ This function should be deprecated, as POST is more secure """
        params = {}
        # if the path is of the form /integer/blah 
        # we encode this as /number/message
        send_regex = re.compile(r"^/(\d+)/(.*)")
        match = send_regex.match(self.path)
        if match:
            # send the message
            if match.group(1): params['number'] = match.group(1)
            if match.group(2): params['message'] = _str(match.group(2))
    ...
    def accept_message(self, params):
        # if no parameters, then start a new session
        ...
        if 'number' in params and 'message' in params:
            # send the message
            ...
            # here we convert the HTTP POST-friendly message (generated by javascript's 'escape') 
            # into something the python apps can interprete
            msg_to_route = urllib.unquote(text) # %20 -> ' '
            msg_to_route = unicode(msg_to_route.replace('%','\\').decode('unicode-escape')) # %u062A -> funky unicode character
